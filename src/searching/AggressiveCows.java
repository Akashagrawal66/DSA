...
public class AggressiveCows {
    public int solve(ArrayList<Integer> A, int B) {
        Collections.sort(A);
        int low = 1;
        int high = (A.get(A.size() - 1) - A.get(0)) / (B - 1);
        int result = high;
        while (low <= high) {
    ...
    private boolean possible(ArrayList<Integer> A, int B, int dist) {
        int required = 1;
        int low = 1;
        int high = A.size() - 1;
        int space = A.get(0) + dist;
        System.out.println("Array is: " + A);
        System.out.println("Size: " + A.size());
        System.out.println("Minimum dist between cows: " + dist);
        while (required < B) {
            int index = binarySearch(A, low, high, space);
            required += 1;
            System.out.println("Index is: " + index);
            System.out.println("Places " + required + " cow at stall " + A.get(index));
            if (required > B || (A.get(index) < space)) {
//                System.out.println(" ");
                return false;
            } else if (required == B) {
                return true;
            ...
            low = index + 1;
            space = A.get(index) + dist;
        }
        return true;
    }

    private int binarySearch(ArrayList<Integer> a, int low, int high, int num) {
        while (low < high) {
            int mid = low + (high - low) / 2;
            if (a.get(mid) < num) {
                low = mid + 1;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(obj.solve(list, 385));
    }
}
